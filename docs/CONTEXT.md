...
* Candidate self-service surface (issue #608): authenticated candidates manage their profile (`GET/PATCH /api/candidate/me`), upload / remove resumes (`POST /api/candidate/me/resume`, `DELETE`), change their password in-session (`POST /auth/me/password` — role-agnostic, revokes every refresh token except the one carrying the current session cookie), and request a GDPR data export (`POST /api/candidate/me/export` enqueues an Arq job; the worker assembles a ZIP with `data.json` + per-application resume snapshots, mints a 24h `DataExportRequest`, and emails a single-use signed link consumed by `GET /api/candidate/me/export/{token}`). Cleanup of expired / used `data_export_request` rows + their storage objects lands in #10's cron.
* Candidate-facing application views (issue #609): `GET /api/candidate/me/applications`, `…/{id}`, and `…/{id}/resume`. The candidate-facing API **never** exposes raw `Application.status` or `admin_notes` and entirely filters out `WITHDRAWN` rows — only the derived `editable` boolean (true iff `status == NEW`) is sent. Foreign-id, withdrawn, and missing-snapshot cases all collapse to a single 404 so the endpoints can't be used to probe other candidates' application IDs. The resume endpoint reuses the storage-streaming helper at `src/api/_resume_streaming.py` shared with the admin resume route.
