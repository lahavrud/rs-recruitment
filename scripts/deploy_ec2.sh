...
SMTP_HOST=$(get_param /rs-recruitment/prod/SMTP_HOST) \
  || { echo "ERROR: SSM parameter not found: SMTP_HOST" >&2; SSM_OK=false; }
SMTP_PORT=$(get_param /rs-recruitment/prod/SMTP_PORT) \
  || { echo "ERROR: SSM parameter not found: SMTP_PORT" >&2; SSM_OK=false; }
SMTP_USER=$(get_param /rs-recruitment/prod/SMTP_USER) \
  || { echo "ERROR: SSM parameter not found: SMTP_USER" >&2; SSM_OK=false; }
SMTP_PASSWORD=$(get_param /rs-recruitment/prod/SMTP_PASSWORD) \
  || { echo "ERROR: SSM parameter not found: SMTP_PASSWORD" >&2; SSM_OK=false; }
SMTP_FROM_EMAIL=$(get_param /rs-recruitment/prod/SMTP_FROM_EMAIL) \
  || { echo "ERROR: SSM parameter not found: SMTP_FROM_EMAIL" >&2; SSM_OK=false; }
...
printf 'SMTP_HOST=%s\n'            "${SMTP_HOST}"             >> "${APP_DIR}/.env"
printf 'SMTP_PORT=%s\n'            "${SMTP_PORT}"             >> "${APP_DIR}/.env"
printf 'SMTP_USER=%s\n'            "${SMTP_USER}"             >> "${APP_DIR}/.env"
printf 'SMTP_PASSWORD=%s\n'        "${SMTP_PASSWORD}"         >> "${APP_DIR}/.env"
printf 'SMTP_FROM_EMAIL=%s\n'      "${SMTP_FROM_EMAIL}"       >> "${APP_DIR}/.env"
