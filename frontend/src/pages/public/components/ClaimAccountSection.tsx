...
interface ClaimAccountSectionProps {
  enabled: boolean;
...
export default function ClaimAccountSection({
  ...
  function validatePassword(val: string): string | null {
    if (val && val.length < 8)
      return t("publicJobs.application.validation.passwordMin");
    return null;
  }
            ...
            <input
              ...
              onChange={(e) => {
                ...
                if (confirmError && passwordConfirm)
                  setConfirmError(
                    validateConfirm(passwordConfirm, e.target.value),
                  );
              }}
