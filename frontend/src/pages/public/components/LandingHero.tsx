...
/* Hero over a plain void: copy on the right (RTL), with the client-logo ribbon
   riding near — but lifted off — the bottom edge. No decorative background. */
export default function LandingHero({
  ...
  return (
    <section className="relative flex h-dvh flex-col overflow-hidden bg-void">
      {/* min-h-0 lets the copy column shrink (and overflow-hidden clip it) so
          the section stays exactly one viewport tall on short/non-maximized
          windows — the ribbon below never spills past the fold. */}
      <div className="relative z-10 mx-auto flex w-full min-h-0 max-w-7xl flex-1 flex-col justify-center overflow-hidden px-6 pb-4 pt-[clamp(5rem,10dvh,8rem)] sm:px-12">
        <div className="max-w-3xl">
          ...
          <div className="font-display mt-[clamp(0.75rem,2.5dvh,1.75rem)] text-[clamp(1.55rem,min(7vw,6dvh),4.25rem)] font-black leading-[1.08] text-white/95">
            ...
            <span className="block text-gold">{t("landing:hero.statementKicker")}</span>
          </div>
