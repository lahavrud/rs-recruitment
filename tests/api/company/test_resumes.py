...
async def test_download_resume_not_found(admin_client: AsyncClient, tmp_path: Path):
    ...
    with patch("src.api._resume_streaming.settings", mock_settings):
...
async def test_download_resume_s3_proxies(admin_client: AsyncClient):
    ...
    with (
        patch("src.api._resume_streaming.settings", mock_settings),
        patch(
            "src.api._resume_streaming.get_storage_provider",
